...
**Goal:** An outside researcher can find AquaPose, understand what it does, install it, run it end-to-end on real data, and cite it — under a correct license, with green CI and live documentation.

**Target features:**
- Branch reconciliation — forward-port the `a66287a` Sphinx repair onto `dev`; `main` becomes release-only
- Relicense MIT → AGPL-3.0 across LICENSE, `pyproject.toml`, docs, README, and citation
- Repo hygiene and green CI — resolve 8 failing tests, remove stray artifacts, consolidate weights-path config
- Two-tier documentation IA — core production pipeline vs. research/beta utilities
- Complete API reference — cover the 52 modules absent from the rendered docs
- Narrative documentation — install guide, concepts page, CLI reference, config reference (71 fields), e2e tutorial
- Zenodo example dataset with DOI — trimmed 12-camera clips, models, calibration, reference outputs
- README refresh — hero media, badge row, install, quick start, citation block
- Read the Docs building green from `dev`
...
---
| Develop on `dev`, `main` release-only | `origin/dev` is 1494 commits ahead; reconciling now is nearly free, later is expensive. `semantic_release` already registers `dev` as a prerelease branch | Decided 2026-08-12 — v4.0 |
| Relicense MIT → AGPL-3.0 | Ultralytics (AGPL-3.0) is unavoidable in every detection and pose backend; leidenalg/igraph add GPL. Advertising MIT misleads downstream users. Conservative option chosen over enterprise licensing or dependency replacement | Decided 2026-08-12 — v4.0 |
