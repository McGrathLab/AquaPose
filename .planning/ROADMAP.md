...
- [x] 111-03-PLAN.md — DATA-03: final checksums.sha256 over the complete tree + verify_deposit completeness/licensing gate, then manual Zenodo upload + DOI mint (human step), DOI recorded in README
...
- [x] 113-01-PLAN.md — TRACER: Getting Started docs section + end-user install guide + drop the cu121 wheel-index pin (DOCS-03)
- [x] 113-02-PLAN.md — CI green-up: stable float64 angular-error metric at all three `acos(dot)` sites (QA-01)
- [x] 113-03-PLAN.md — Zenodo deposit factual corrections in template and tree, plus regression tests and re-emitted checksums (DOCS-07, DATA-01)

**Wave 2** *(blocked on Wave 1 completion)*

- [x] 113-04-PLAN.md — Concepts page: five pipeline stages, refraction, and the real B-spline midline representation (DOCS-04)
- [x] 113-05-PLAN.md — End-to-end tutorial verification run on the deposit: timings, measured statistics, deviation fixes (DOCS-07)
...
**Plans:** 7/7 plans executed (wave 1: 01-04 in parallel; wave 2: 06, 07; wave 3: 05)
...
- [x] 113.1-07-PLAN.md — declare `pillow` as a real dependency; stop `CLAUDE.md` feeding the drifted GUIDEBOOK into planning

**Wave 3** *(blocked on Wave 2 completion)*

- [x] 113.1-05-PLAN.md — YH t-values recomputed on the 322-crop manual corpus, archive gap recorded, phase close-out against a green suite
...
**Re-measured 2026-09-02 at planning time:** still 98 errors, per-file distribution identical
to `113.2-CONTEXT.md`. Grouped by root cause rather than by file: **86 h5py union-narrowing
errors + a 12-error tail** (CONTEXT D-01 estimated 89/9; the extra three are the two
...
- [x] 113.2-04-PLAN.md — Apply the helper across `core/`: `reid/runner.py` (27 → 0) and `stitching.py` (12 → 0)
- [x] 113.2-05-PLAN.md — `reid/swap_detector.py` (7 → 0), including a written verdict on the line-310 optional access (D-06)
- [x] 113.2-06-PLAN.md — Terminal gate: audit against pre-phase suppression baselines, record the D-07 decision, confirm CI green on `dev`
...
- [x] 114-01-PLAN.md — D-13 URL hygiene (`CODE_OF_CONDUCT.md`, `docs/contributing.md`) and README-03 deferral bookkeeping (REQUIREMENTS + ROADMAP annotation + backlog todo)
- [x] 114-02-PLAN.md — Activate Read the Docs from `dev` (DOCS-08) and confirm the Codecov project link; record every badge/funnel URL as verified
- [x] 114-03-PLAN.md — Version reset to `4.0.0` (D-01), CHANGELOG migration note, and `CITATION.cff` Pass A with no `doi` key (D-11)
